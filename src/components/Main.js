...
import ChatBot from "./widgets/ChatBot.js";
import SkillsSection from "./widgets/SkillSection.js";
...
const MainForm = () => {
  ...
  useEffect(() => {
    const showTimer = setTimeout(() => setShowHint(true), 1000); // show after 1 sec
    const hideTimer = setTimeout(() => setShowHint(false), 6000); // hide after 5 sec
  ...
  const data = [
    {
      name: "Quick Samachar",
      tags: ["Flutter", "Provider", "Firebase", "REST API"],
    },
    {
      name: "Medha App",
      tags: ["Flutter", "BLoC", "Firebase", "Cloud Firestore"],
    },
  ...
  return (
    <div
      className={`min-h-screen ${colorTheme.bgGradient} ${colorTheme.textBody} font-sans`}
    >
   <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end space-y-2">
  {/* Hint bubble */}
  {showHint && (
    <div className="bg-white border border-gray-300 shadow-xl rounded-xl px-4 py-2 text-sm text-gray-800 animate-bounce max-w-xs">
      💬 Click here to chat with <span className="font-semibold text-indigo-600">UtsavBot</span>!
    </div>
  )}
  ...
  <div
    ...
    className="w-14 h-14 bg-indigo-600 text-white text-2xl rounded-full shadow-lg hover:bg-indigo-700 cursor-pointer transition-all duration-300 flex items-center justify-center"
  >
    💬
  </div>
</div>
            ...
            <div className="md:hidden">
              <button
                ...
                className={`p-2 rounded-md ${
                  isScrolled ? colorTheme.textPrimary : colorTheme.textAccent
                }`}
              >
                <svg
              ...
              >
                ...
                <ul className="space-y-2 pt-2">
                  <li className="flex items-center">
                    <span className={`mr-2 ${colorTheme.textAccent} text-xl`}>
                      ▹
                    </span>
...
<SkillsSection colorTheme={colorTheme} />
      </motion.section>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            Featured Projects
            <span
              className={`block w-20 h-1 ${colorTheme.gradientAccentBar} mx-auto mt-2 rounded`}
            ></span>
          </motion.h2>
          <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {data.map((project, index) => (
              <motion.div
                ...
                whileHover={{
                  y: -10,
                  boxShadow: "0 20px 40px -15px rgba(100, 116, 139, 0.25)",
                }}
              ...
              >
                ...
                <div
                  className={`h-48 ${colorTheme.bgPlaceholder} flex items-center justify-center relative group overflow-hidden`}
                >
                ...
                <div className="p-6 flex-grow flex flex-col">
                  <h3
                    className={`text-xl font-semibold mb-2 ${colorTheme.textPrimary}`}
                  >
                    {project.name}
                  </h3>
                  <p
                    className={`${colorTheme.textSubtle} mb-4 text-sm flex-grow`}
                  >
      ...
      <footer
        className={`py-8 ${colorTheme.bgFooter} border-t ${colorTheme.borderStandard}`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
